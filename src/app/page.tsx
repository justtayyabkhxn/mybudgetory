...
const features = [
  ...
  {
    icon: <CalendarCheck className="h-8 w-8 text-pink-600 dark:text-pink-400" />,
    title: "Goal Planning",
...
const ModeCard = ({ href, icon, title, bg, color, desc }: Mode) => (
  <Link href={href} className="block group">
    ...
    >
      ...
      <div className="relative z-10">
        <div className={`${bg} w-fit mx-auto p-5 rounded-2xl mb-6 text-white shadow-lg transform group-hover:scale-110 transition-transform duration-300`}>
          {icon}
        </div>
        <h3 className={`text-2xl font-bold mb-4 ${color} flex items-center justify-center gap-2`}>
          {title}
...
const TestimonialCard = ({ text, author }: Testimonial) => (
  ...
  >
    ...
    <p className="italic text-gray-700 dark:text-gray-300 mb-4 leading-relaxed">{text}</p>
    <footer className="font-semibold text-gray-900 dark:text-white">– {author}</footer>
  </motion.blockquote>
);
